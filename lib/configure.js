...
var fs = require('fs'),
  path = require('path'),
  registry = require('../node_modules/windows/lib/registry'),
  execSync = require('../node_modules/windows/lib/Command').execSync,
...
module.exports = function (settings, logger) {
  ...
  git.cmd = path.join(basedir, 'git', 'cmd', 'git.cmd');
  ...
  if (!path.existsSync(settings.git.cmd)) {
    if (path.existsSync(git.cmd)) {
      settings.git.cmd = git.cmd;
    } else {
...
//Remove sync files (workaround around the impossibility to unlink in real time working files generated by the execSync
